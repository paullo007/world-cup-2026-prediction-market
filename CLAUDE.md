...
| `lib/ingest.ts` | `ingestAndPublish()` — shared detect + **auto-resolve/pay out** (no approval gate); used by cron + button |
| `app/api/cron/results/` | Vercel Cron (CRON_SECRET-gated): calls `ingestAndPublish()` + stamps the 60-min cooldown |
| `app/api/refresh-results/` | Public, logged-in-only "Update Latest Results" trigger: atomic 60-min cooldown claim → `ingestAndPublish()` |
