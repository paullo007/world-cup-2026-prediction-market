...
| `components/CountryDetail.tsx`, `components/SquadTable.tsx` | Shared country layout (Brazil + every `/countries/[slug]` render identically) + sticky-header squad table. Group-stage fixtures: played games show **FT score (oriented to the country) + Won/Lost/Draw** and a red bold **"COMPLETED"** badge top-right; upcoming games get a **bold green edge** (`border-2 border-yes`). Pages pass a `results` map from `resultsForCountry()` (lib/countries) |
