...
- **ESPN scoring-play types: penalties are "Penalty - Scored" (no "goal").** Parse goals with `/goal|penalty/i`, not `/goal/i`, or penalty goals vanish (own goals stay excluded via `ownGoal`; missed penalties via `scoringPlay=false`). When a goal turns out missing, sanity-check that **scorers reconcile with the score**.
